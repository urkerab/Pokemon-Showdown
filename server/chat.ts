	...
	checkChat(message: string | null = null, room: Room | null = null, targetUser: User | null = null) {
		...
		if (!user.can('bypassall')) {
			...
			if (room) {
				...
				if (room.users && !(user.id in room.users)) {
...
export const Chat = new class {
	...
	getDataNatureHTML(nature: AnyObject) {
		if (typeof nature === 'string') nature = Dex.getNature(nature);
		let buf = `<ul class="utilichart"><li class="result">`;
	...
	describeThing(thing: AnyObject, dex: ModdedDex = Dex) {
		if (thing.effectType === 'Pokemon') {
			...
			*/
			...
			if (dex.gen > 2 && thing.abilities['1'] && (dex.gen > 3 || dex.getAbility(thing.abilities['1']).gen === 3)) {
