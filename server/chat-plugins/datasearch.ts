...
function runDexsearch(target: string, cmd: string, canAll: boolean, message: string, isTest: boolean) {
	...
	for (const andGroup of splitTarget) {
		...
		if (parameters.length > 4) return {error: "No more than 4 alternatives for each parameter may be used."};
	...
	for (const pokemon in mod.data.Pokedex) {
		...
		const prevoSearchResult = (
			prevoSearch === null ||
			prevoSearch === (species.prevo !== '' && mod.getSpecies(species.prevo).gen <= mod.gen)
		);
		const evoSearchResult = (
			evoSearch === null ||
			evoSearch === species.evos.some(evo => mod.getSpecies(evo).gen <= mod.gen)
		);
...
function runMovesearch(target: string, cmd: string, canAll: boolean, message: string, isTest: boolean) {
	...
	const allFlags = [
		'authentic', 'bite', 'bullet', 'charge', 'contact', 'dance', 'defrost', 'distance', 'gravity', 'highcrit', 'mirror',
