...
export class RandomTeams {
	...
	statCalc(baseStats: StatsTable, ivs: StatsTable | number, evs: StatsTable | number, level: number, nature: string) {
		...
		const n = this.dex.natures.get(nature);
		if (n.plus) {
	...
	levelBalance(baseStats: StatsTable, ivs: StatsTable | number, evs: StatsTable | number, nature: string,
		...
		const worstStats = this.statCalc(this.dex.species.get(weakling).baseStats, averageIV, averageEV, 100, 'Serious');
	...
	randomLBCCTeam(): RandomTeamsTypes.RandomSet[] {
		const dex = this.dex;
		const team = [];
	...
	randomHCTeam(): PokemonSet[] {
		...
		if (doItemsExist) {
			if (!hasCustomBans) {
				...
				if (this.format.id.includes("mixandmega")) {
					itemPool = itemPool.filter(item => item.megaStone);
				}
			} else {
	...
	randomSet(
		...
		const movePool = randMoves ? randMoves.slice() : Object.keys(this.dex.species.getLearnset(species.id)!);
		...
		while (pokemonPool.length && pokemon.length < this.maxTeamSize) {
			...
			if (!isMonotype) {
				...
				for (const typeName of this.dex.types.names()) {
					...
					if (this.dex.getEffectiveness(typeName, template) > 0) {
			...
			const set = this.randomSet(template, teamDetails, pokemon.length === 0,
