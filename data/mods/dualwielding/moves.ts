export const Moves: {[k: string]: ModdedMoveData} = {
	...
	"knockoff": {
		...
		onBasePower(basePower, source, target, move) {
			...
			if (item.id && this.singleEvent('TakeItem', item, target.itemState, target, source, move, item)) {
				return this.chainModify(1.5);
			}
			const dual = target.getAbility() as any as Item;
			if (dual.id && dual.effectType === 'Item' && this.singleEvent('TakeItem', dual, target.abilityState, target, source, move, dual)) {
				return this.chainModify(1.5);
			}
		},
	...
	"switcheroo": {
		...
		onHit(target, source, move) {
			...
			} else if ((myItem && !this.singleEvent('TakeItem', myItem, source.itemState, target, source, move, myItem)) || (yourItem && !this.singleEvent('TakeItem', yourItem, target.itemState, source, target, move, yourItem))) {
				if (yourItem) target.item = yourItem.id;
				if (myItem) source.item = myItem.id;
			} else {
			...
			} else if ((myDual && !this.singleEvent('TakeItem', myDual, source.abilityState, target, source, move, myDual)) || (yourDual && !this.singleEvent('TakeItem', yourDual, target.abilityState, source, target, move, yourDual))) {
				if (yourDual) target.baseAbility = target.ability = yourDual.id;
				if (myDual) source.baseAbility = source.ability = myDual.id;
			} else {
				this.add('-activate', source, 'move: Trick', '[of] ' + target);
