export const Moves: {[k: string]: ModdedMoveData} = {
		...
		onHit(target, source, move) {
			...
			for (const sideCondition of removeAll) {
				if (source.side.removeSideCondition(sideCondition)) {
					this.add('-sideend', source.side, this.dex.getEffect(sideCondition).name, '[from] move: Cleansing Fire', '[of] ' + source);
					success = true;
				}
				if (target.side.removeSideCondition(sideCondition)) {
					this.add('-sideend', target.side, this.dex.getEffect(sideCondition).name, '[from] move: Cleansing Fire', '[of] ' + source);
					success = true;
				}
			}
			return success;
	...
	"sacredcurrent": {
		...
		flags: {snatch: 1},
		onHit(target) {
			const statNames: StatNameExceptHP[] = ['atk', 'def', 'spa', 'spd', 'spe'];
			const highestStats: StatNameExceptHP[] = [];
			let bestStat = 0;
			for (let i = 0; i < 2; i++) {
				for (let j = 0; j < 5; j++) {
					const statName: StatNameExceptHP = statNames[j];
