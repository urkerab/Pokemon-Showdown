...
export class RandomSeasonalTeams extends RandomGen6Teams {
	randomTeam() {
		const team = [];
		const variant = (this.random(2) === 1);
		const sets: {[name: string]: SSBSet} = {
			'Acast': {
			...
			'Antemortem': {
				species: 'Clefable', ability: (variant ? 'Sheer Force' : 'Multiscale'), item: (variant ? 'Life Orb' : 'Leftovers'), gender: 'M',
			...
			'Anty': {
				species: 'Durant', ability: 'Hustle', item: (variant ? 'Life Orb' : 'Choice Band'), gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)],
			...
			'Blast Chance': {
				species: 'Malamar', ability: 'Contrary', item: 'Leftovers', gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)],
			...
			'CoolStoryBrobat': {
				...
				signatureMove: "Brave Bat",
				evs: {hp: 4, atk: 252, spe: 252}, nature: 'Jolly',
			},
			'Crestfall': {
				species: 'Diancie', ability: 'Shield Dust', item: 'Diancite', gender: 'N',
			...
			'DMT': {
				species: 'Shedinja', ability: 'Wonder Guard', item: 'Focus Sash', gender: 'N',
			...
			'E4 Flint': {
				species: 'Dialga', ability: 'Tinted Lens', item: 'Adamant Orb', gender: 'N',
			...
			'Giagantic': {
				species: 'Moltres', ability: 'Pressure', item: 'Leftovers', gender: 'N', shiny: true,
				moves: ['fierydance', 'aeroblast', 'roost'],
				signatureMove: "Eternal Ashes",
				evs: {hp: 248, spd: 244, spe: 16}, nature: 'Bold',
			},
			'Golui': {
				species: 'Cryogonal', ability: 'Special Snowflake', item: 'Life Orb', gender: 'N',
			...
			'grimAuxiliatrix': {
				species: 'Krookodile', ability: 'Intimidate', item: 'Choice Band', gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)],
			...
			'jdarden': {
				...
				moves: ['rest', 'sleeptalk', 'quiverdance'],
			...
			'Pikachuun': {
				species: 'Pikachu', ability: 'Magic Guard', item: 'Light Ball', gender: 'M',
				moves: ['thunderbolt', 'lightofruin', 'icebeam'],
				signatureMove: "Pure Skill",
				evs: {def: 4, spa: 252, spe: 252}, nature: 'Modest',
			},
			'pluviometer': {
				species: 'Mismagius', ability: 'Levitate', item: 'Life Orb', gender: 'M',
			...
			'Raseri': {
				species: 'Musharna', ability: 'Synchronize', item: 'Leftovers', gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)],
			...
			'scpinion': {
				species: 'Slowbro', ability: 'Unaware', item: 'Slowbronite', gender: 'M',
			...
			'Sonired': {
				species: 'Anorith', ability: 'Sniper', item: 'Choice Band', gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)],
			...
			'Starmei': {
				species: 'Staryu', ability: 'Parental Bond', item: 'Leftovers', gender: 'N', shiny: true,
			...
			'Teremiare': {
				species: 'Zorua', ability: 'Multiscale', item: 'Red Card', gender: (['M', 'F', 'N'] as GenderName[])[this.random(3)], shiny: true,
		...
		for (let i = 0; i < 6; i++) {
			...
			const set = {name, ivs: {}, level: 100, shiny: false, ...sets[name]};
			set.ivs = {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31, ...set.ivs};
			if (set.moves.length > 3) set.moves = [this.sampleNoReplace(set.moves), this.sampleNoReplace(set.moves), this.sampleNoReplace(set.moves)];
			set.moves.push(set.signatureMove);
			team.push(set);
		}
...
export default RandomSeasonalTeams;
