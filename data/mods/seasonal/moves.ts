export const Moves: {[k: string]: ModdedMoveData} = {
	...
	brokenwand: {
		...
		onHit(target, source) {
			if (this.random(100) < 20) {
				...
				this.damage(source.maxhp * 0.75, source, source, this.dex.getEffect('Broken Wand'));
				return false;
			}
		...
		onHit(target, source) {
			...
			for (const i in sideConditions) {
				if (source.side.removeSideCondition(i)) {
					this.add('-sideend', source.side, this.dex.getEffect(i).name, '[from] move: Buzz Axe Rampage', '[of] ' + source);
		...
		onAfterMoveSecondarySelf(source, target) {
			...
			for (const targetCondition of removeTarget) {
				if (target.side.removeSideCondition(targetCondition)) {
					...
					this.add('-sideend', target.side, this.dex.getEffect(targetCondition).name, '[from] move: Defog', '[of] ' + target);
				}
			}
		...
		onHit(target, source) {
			...
			for (const targetCondition of removeTarget) {
				...
				if (foe.removeSideCondition(targetCondition)) {
					if (!removeAll.includes(targetCondition)) continue;
					this.add('-sideend', foe, this.dex.getEffect(targetCondition).name, '[from] move: Defog', '[of] ' + foe.active[0]);
				}
			}
			for (const sideCondition of removeAll) {
				if (source.side.removeSideCondition(sideCondition)) {
		...
		condition: {
			...
			onEnd(side) {
				...
				if (side.active.length && side.active[0].hp && foes.length && foes[0].hp) {
					foes[0].addVolatile('trapped', side.active[0], this.dex.getEffect('meanlook'), 'trapper');
				}
			},
		},
		boosts: {
		...
		condition: {
			...
			onBeforeSwitchOut(pokemon) {
				...
				for (const source of this.effectData.sources) {
					...
					this.runMove('predator', source, this.getTargetLoc(pokemon, source));
				}
			},
		},
		target: "normal",
	...
	starboltdesperation: {
		...
		onHit(target, source) {
			...
			const stats: BoostName[] = ['atk', 'def', 'spa', 'spd', 'spe', 'accuracy'];
	...
	sweetdreams: {
		...
		condition: {
			...
			onResidualOrder: 8,
			onResidual(pokemon) {
				const target = this.effectData.source.side.active[pokemon.volatiles['sweetdreams'].sourcePosition];
	...
	wonderbark: {
		...
		onHit(pokemon, source) {
			...
			for (let i = 0; i < pokemon.moveSlots.length; i++) {
				...
				const moveData = this.dex.getMove(newMoves[index]);
