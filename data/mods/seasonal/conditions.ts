export const Conditions: {[k: string]: ModdedConditionData} = {
		...
		onResidual(pokemon) {
			...
			if (thisSide.foe.active.length) {
				...
				if (opponent.hp && opponent.status === 'slp') {
					...
					this.damage(opponent.maxhp / 8, opponent, pokemon, this.dex.getEffect("Bad Dreams"));
				}
			}
		},
	},
	...
	coinflip: {
		...
		onStart(target, source) {
			...
			const pool: BoostName[] = ['atk', 'def', 'spa', 'spd', 'spe'];
	...
	prodigy: {
		...
		onStart(target, source) {
			...
			this.field.addPseudoWeather('prodigyweather', source, this.dex.getEffect("Prodigy"));
		},
		onSwitchOut(pokemon) {
