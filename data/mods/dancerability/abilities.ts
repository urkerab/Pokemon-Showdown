	...
	*/
	...
	damp: {
		...
		onAnyTryMove(target, source, effect) {
			if (effect.id === 'selfdestruct' || effect.id === 'explosion') {
				...
				this.bounceMove(effect, this.effectState.target, target);
				return false;
			}
		},
	},
	...
	flowerveil: {
		...
		onAllyBoost(boost, target, source, effect) {
			...
			if (showMsg && !(effect as ActiveMove).secondaries) {
				this.add('-fail', this.effectState.target, 'unboost', '[from] ability: Flower Veil', '[of] ' + target);
				this.bounceMove(effect, target, source);
			}
		},
	...
	illusion: {
		...
		onDamagingHit(damage, target, source, move) {
			if (target.illusion && move && move.effectType === 'Move' && move.id !== 'confused') {
				this.singleEvent('End', this.dex.abilities.get('Illusion'), target.abilityState, target, target);
				this.bounceMove(move, target, source);
			}
		},
	},
	...
	*/
	stickyhold: {
		...
		onTakeItem(item, pokemon, source) {
			if (this.suppressingAbility(pokemon) && pokemon !== this.activePokemon || !pokemon.hp || pokemon.item === 'stickybarb') return;
