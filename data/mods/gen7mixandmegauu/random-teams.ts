...
export class RandomMixAndMegaUUGen7Teams extends RandomGen7Teams {
	randomTeam() {
		...
		while (pokemonPool.length && pokemonLeft < 6) {
			...
			if (template.randomBattleMoves) {
				...
				if (this.dex.getLearnsetData(template.id).learnset!.switcheroo || this.dex.getLearnsetData(template.id).learnset!.trick) {
					const newTemplate = this.dex.deepClone(template);
			...
			if (!template.requiredItem && template.tier !== 'Uber' && template.tier !== 'OU' && !template.evos.length && set.item !== 'Thick Club' && set.item !== 'Stick') {
				...
				if (stone) {
					...
					const mega = stone === 'Blue Orb' ? 'kyogreprimal' : stone === 'Red Orb' ? 'groudonprimal' : toID(this.dex.getItem(stone).megaStone);
...
export default RandomMixAndMegaUUGen7Teams;
