	...
	*/
	...
	ev: {
		...
		onStart(target) {
			...
			const forme = Object.keys(formes)[this.random(8)];
	...
	lostseso: {
		...
		onStart() {
			...
			this.effectState.danceMultiplier = 0;
		},
		onAfterMove(pokemon, target, move) {
			if (move.flags.dance) this.effectState.danceMultiplier++;
		},
		onSwitchOut() {
		...
		onStart(source) {
			...
			for (const sideCondition of removeAll) {
				if (target.side.removeSideCondition(sideCondition)) {
					if (!(silentRemove.includes(sideCondition))) this.add('-sideend', target.side, this.dex.conditions.get(sideCondition).name, '[from] move: No Fun Zone', '[of] ' + source);
				}
				if (source.side.removeSideCondition(sideCondition)) {
					if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side, this.dex.conditions.get(sideCondition).name, '[from] move: No Fun Zone', '[of] ' + source);
				}
			}
	...
	hail: {
		...
		onStart(battle, source, effect) {
			if (effect && effect.effectType === 'Ability') {
				if (this.gen <= 5 || effect.id === 'snowstorm') this.effectState.duration = 0;
