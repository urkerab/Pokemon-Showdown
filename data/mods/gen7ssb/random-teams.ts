...
export class RandomStaffBrosTeams extends RandomGen7Teams {
	randomStaffBrosTeam() {
		...
		const typePool: {[type: string]: number} = {};
		while (pool.length && team.length < 6) {
			const name = this.sampleNoReplace(pool);
			const ssbSet = sets[name];
			// Enforce typing limits
			let types = this.dex.species.get(ssbSet.species).types;
			...
			if (ssbSet.ivs) {
				...
				for (iv in ssbSet.ivs) {
					...
					// @st-ignore StatsTable has no index signature
