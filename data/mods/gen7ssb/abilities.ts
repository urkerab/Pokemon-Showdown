	...
	*/
	...
	logia: {
		...
		onTryHit(target, source, move) {
			const plateType = this.dex.items.get(target.item).onPlate;
	...
	stimulatedpride: {
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.side.foe.active) {
				if (!target?.isAdjacent(pokemon)) continue;
				if (!activated) {
				...
				if (target.volatiles['substitute']) {
					...
					this.boost({atk: -1}, target, pokemon, this.dex.abilities.get('intimidate'));
				}
			}
		},
	...
	gracideamastery: {
		...
		onAfterMove(pokemon, move) {
			if (pokemon.species.baseSpecies !== 'Shaymin' || pokemon.transformed) return;
	...
	fakecrash: {
		...
		onDamage(damage, target, source, effect) {
			if (effect && effect.effectType === 'Move' && target.species.id === 'lycanrocmidnight' && !target.transformed) {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (!target) return;
			if (target.species.id !== 'lycanrocmidnight' || target.transformed || (target.volatiles['substitute'] && !(move.flags['authentic'] || move.infiltrates))) return;
			if (!target.runImmunity(move.type)) return;
			return 0;
		},
		onUpdate(pokemon) {
			if (pokemon.species.id === 'lycanrocmidnight' && this.effectState.busted) {
				const templateid = 'Lycanroc-Dusk';
	...
	beastboost2: {
		...
		onSourceFaint(target, source, effect) {
			if (effect && effect.effectType === 'Move') {
				// @st-ignore
				const statOrder = (Object.keys(source.storedStats) as StatIDExceptHP[]).sort((stat1, stat2) => source.storedStats[stat2] - source.storedStats[stat1]);
	...
	snowstorm: {
		...
		isNonstandard: "Custom",
		onStart() {
			const snowStorm = this.dex.deepClone(this.dex.conditions.get('hail'));
