export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	*/
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.side.foe.active) {
				...
				if (target.volatiles['substitute']) {
					...
					this.boost({atk: -1}, target, pokemon, this.dex.getAbility('intimidate'));
				}
			}
		},
	...
	fakecrash: {
		...
		onUpdate(pokemon) {
			if (pokemon.species.id === 'lycanrocmidnight' && this.effectData.busted) {
	...
	beastboost2: {
		...
		onSourceFaint(target, source, effect) {
			if (effect && effect.effectType === 'Move') {
				// @st-ignore
				const statOrder = (Object.keys(source.storedStats) as StatNameExceptHP[]).sort((stat1, stat2) => source.storedStats[stat2] - source.storedStats[stat1]);
	...
	snowstorm: {
		...
		isNonstandard: "Custom",
		onStart() {
			const snowStorm = this.dex.deepClone(this.dex.getEffect('hail'));
