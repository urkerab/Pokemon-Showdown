	...
	*/
		...
		onHit(target, source, move) {
			...
			for (const targetCondition of removeTarget) {
				if (target.side.removeSideCondition(targetCondition)) {
					...
					this.add('-sideend', target.side, this.dex.conditions.get(targetCondition).name, '[from] move: Noble Howl', '[of] ' + target);
				}
			}
			for (const sideCondition of removeAll) {
				if (source.side.removeSideCondition(sideCondition)) {
					this.add('-sideend', source.side, this.dex.conditions.get(sideCondition).name, '[from] move: Noble Howl', '[of] ' + source);
				}
			}
		},
			...
			if (decision) {
				...
				if (move.category === 'Status' && move.id !== 'mefirst' && move.target) {
					if (['all', 'adjacentAllyOrSelf', 'allySide', 'allyTeam', 'self'].includes(move.target)) {
						 this.actions.useMove(move, source, source);
					} else {
		...
		onHit(target, source, move) {
			...
			for (const sideCondition of removeAll) {
				if (target.side.removeSideCondition(sideCondition)) {
					if (!(silentRemove.includes(sideCondition))) this.add('-sideend', target.side, this.dex.conditions.get(sideCondition).name, '[from] move: Blustery Winds', '[of] ' + source);
				}
				if (source.side.removeSideCondition(sideCondition)) {
					if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side, this.dex.conditions.get(sideCondition).name, '[from] move: Blustery Winds', '[of] ' + source);
				}
			}
	...
	wondertrade: {
		...
		onHit(target, source) {
			...
			const carryOver = [];
			const currentTeam = source.side.pokemon;
			for (const pokemon of currentTeam) {
				carryOver.push({
					hp: pokemon.hp / pokemon.maxhp,
					status: pokemon.status,
					statusState: pokemon.statusState,
					pp: pokemon.moveSlots.slice().map(m => m.pp / m.maxpp),
				});
			...
			// Generate a new team
			const team = this.teamGenerator.getTeam({name: source.side.name});
	...
	ancestralpower: {
		...
		onHit(target, source, move) {
			...
			source.formeChange(forme, this.dex.abilities.get('psychicsurge'), true);
			this.boost({atk: 1, spa: 1}, source, source, move);
			this.actions.useMove(formes[forme], source, target);
			this.boost({atk: -1, spa: -1}, source, source, move);
			source.formeChange(baseForme, this.dex.abilities.get('psychicsurge'), true);
		},
		secondary: null,
		target: "normal",
		type: "Psychic",
	...
	evoblast: {
		...
		onAfterMoveSecondarySelf(pokemon) {
			const stat = (['atk', 'def', 'spa', 'spd', 'spe', 'accuracy'] as BoostID[])[this.random(6)];
			const boost: SparseBoostsTable = {};
			boost[stat] = 1;
	...
	purplepills: {
		...
		condition: {
			...
			onStart(pokemon) {
				...
				const newMovep = [], statMove = [], offMove1 = [];
				let offMove2 = [];
				// isViable no longer exists, so this is the list of viable moves
				const viableMoves = [
					'accelerock', 'acidspray', 'acrobatics', 'aerialace', 'aeroblast', 'agility', 'airslash', 'anchorshot', 'aquajet', 'aquatail',
					'aromatherapy', 'attackorder', 'aurasphere', 'auroraveil', 'autotomize', 'avalanche', 'banefulbunker', 'beakblast', 'blazekick',
					'blizzard', 'blueflare', 'bodyslam', 'boltstrike', 'bonemerang', 'boomburst', 'bravebird', 'brickbreak', 'bugbuzz', 'bulkup',
					'bulletpunch', 'bulletseed', 'calmmind', 'chatter', 'circlethrow', 'clangingscales', 'clearsmog', 'closecombat', 'coil', 'coreenforcer',
					'cottonguard', 'crabhammer', 'crosschop', 'crunch', 'darkestlariat', 'darkpulse', 'darkvoid', 'dazzlinggleam', 'defendorder', 'defog',
					'destinybond', 'detect', 'diamondstorm', 'disable', 'discharge', 'doubleedge', 'dracometeor', 'dragonascent', 'dragonclaw', 'dragondance',
					'dragonhammer', 'dragonpulse', 'dragontail', 'drainpunch', 'drillpeck', 'drillrun', 'earthpower', 'earthquake', 'encore', 'endeavor',
					'energyball', 'eruption', 'explosion', 'extrasensory', 'extremespeed', 'facade', 'fakeout', 'fierydance', 'finalgambit', 'fireblast',
					'firefang', 'firelash', 'firepunch', 'firstimpression', 'flamecharge', 'flamethrower', 'flareblitz', 'flashcannon', 'fleurcannon',
					'focusblast', 'focuspunch', 'foulplay', 'freezedry', 'frustration', 'fusionbolt', 'fusionflare', 'geargrind', 'geomancy', 'gigadrain',
					'glare', 'grassknot', 'gunkshot', 'gyroball', 'hammerarm', 'haze', 'headcharge', 'headsmash', 'healbell', 'healingwish', 'healorder',
					'heatwave', 'heavyslam', 'hex', 'highjumpkick', 'honeclaws', 'hornleech', 'hurricane', 'hydropump', 'hyperspacefury', 'hypervoice',
					'icebeam', 'icefang', 'icehammer', 'icepunch', 'iceshard', 'iciclecrash', 'iciclespear', 'ironhead', 'judgment', 'jumpkick',
					'kingsshield', 'knockoff', 'lavaplume', 'leafblade', 'leafstorm', 'leechlife', 'leechseed', 'lightofruin', 'lightscreen', 'liquidation',
					'lovelykiss', 'lowkick', 'lunardance', 'lunge', 'machpunch', 'magiccoat', 'magmastorm', 'megahorn', 'memento', 'meteormash', 'milkdrink',
					'mindblown', 'moonblast', 'moongeistbeam', 'moonlight', 'morningsun', 'multiattack', 'nastyplot', 'naturepower', 'naturesmadness',
					'nightshade', 'nightslash', 'nuzzle', 'oblivionwing', 'originpulse', 'outrage', 'overheat', 'painsplit', 'partingshot', 'perishsong',
					'petalblizzard', 'photongeyser', 'plasmafists', 'playrough', 'poisonjab', 'powergem', 'poweruppunch', 'powerwhip', 'precipiceblades',
					'protect', 'psychic', 'psychicfangs', 'psychoboost', 'psychocut', 'psyshock', 'psystrike', 'pursuit', 'quickattack', 'quiverdance',
					'rapidspin', 'razorshell', 'recover', 'reflect', 'refresh', 'relicsong', 'rest', 'return', 'revelationdance', 'roar', 'rockblast',
					'rockpolish', 'rockslide', 'roost', 'sacredfire', 'sacredsword', 'scald', 'searingshot', 'secretsword', 'seedbomb', 'seedflare',
					'seismictoss', 'shadowball', 'shadowbone', 'shadowclaw', 'shadowforce', 'shadowpunch', 'shadowsneak', 'shellsmash', 'shiftgear',
					'shoreup', 'signalbeam', 'slackoff', 'sleeppowder', 'sleeptalk', 'sludgebomb', 'sludgewave', 'softboiled', 'spacialrend', 'spectralthief',
					'spikes', 'spikyshield', 'spiritshackle', 'spore', 'stealthrock', 'steameruption', 'stickyweb', 'stoneedge', 'stormthrow', 'strengthsap',
					'substitute', 'suckerpunch', 'sunsteelstrike', 'superfang', 'superpower', 'surf', 'switcheroo', 'swordsdance', 'synthesis', 'tailglow',
					'tailslap', 'tailwind', 'taunt', 'technoblast', 'thousandarrows', 'thunder', 'thunderbolt', 'thunderpunch', 'thunderwave', 'toxic',
					'toxicspikes', 'toxicthread', 'triattack', 'trick', 'uturn', 'vcreate', 'voltswitch', 'volttackle', 'waterfall', 'watershuriken',
					'waterspout', 'whirlwind', 'wildcharge', 'willowisp', 'wish', 'woodhammer', 'xscissor', 'zenheadbutt', 'zingzap',
				];
				for (const id of viableMoves) {
					const move = this.dex.data.Moves[id];
					if (move.type && !pokemon.types.includes(move.type)) continue;
					// Time to sort!
					if (move.category === 'Status') statMove.push(id);
			...
			onRestart(pokemon) {
				...
				// track percentages to keep purple pills from resetting pp
				pokemon.m.ppPercentages = pokemon.moveSlots.slice().map(m => m.pp / m.maxpp);
				// Update movepool
				const newMovep = ['moonlight', 'heartswap', 'batonpass', 'purplepills'];
				pokemon.moveSlots = [];
				for (const [i, moveid] of newMovep.entries()) {
					const move = this.dex.moves.get(moveid);
					if (!move.id) continue;
	...
	hazardpass: {
		...
		onHitSide(target, source) {
			...
			const hazards: {[key: string]: number} = {stealthrock: 1, spikes: 3, toxicspikes: 2, stickyweb: 1};
			...
			this.add('-anim', source, this.dex.moves.get(hazard1).name, target);
			...
			this.add('-anim', source, this.dex.moves.get(hazard2).name, target);
		...
		condition: {
			...
			onTryHit(target, source, effect) {
				...
				if (this.random(20) === 1) {
					...
					this.actions.useMove('Glitch Out', source, source.side.foe.active[0]);
					return null;
				}
			},
	...
	glitchout: {
		...
		onHit(target, source, effect) {
			const moves = [];
			for (const i in this.dex.data.Moves) {
				const move = this.dex.data.Moves[i];
				if (move.realMove) continue;
				if (move.isZ || move.isNonstandard) continue;
				if (effect.noMetronome && effect.noMetronome.includes(i)) continue;
				if (this.dex.moves.get(i).gen > this.gen) continue;
				moves.push({id: i, num: move.num});
			}
			let randomMove = '';
			if (moves.length) {
			...
			if (napWeather.source === target) {
				for (const curMon of this.getAllActive()) {
					...
					if (curMon.status !== 'slp' && curMon.status !== 'frz' && !curMon.hasAbility('comatose')) {
						...
						this.actions.useMove(move, curMon, curMon, move);
					}
				}
			}
	...
	typeanalysis: {
		...
		onHit(target, source) {
			...
			const species = this.dex.species.get('Silvally-' + randomType);
			source.formeChange(species, this.dex.abilities.get('rkssystem'), true);
			...
			onHit(pokemon) {
				...
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
						this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Cyclone Spin', '[of] ' + pokemon);
					}
				}
		...
		condition: {
			...
			onModifyWeight(weight, pokemon) {
				if (this.effectState.multiplier) {
					weight += this.effectState.multiplier * 100;
					return weight;
				}
			},
		},
	...
	prismaticterrain: {
		...
		condition: {
			...
			onTryMove(target, source, move) {
				const hazardMoves = ['reflect', 'lightscreen', 'auroraveil', 'safeguard', 'mist', 'spikes', 'toxicspikes', 'stealthrock', 'stickyweb', 'hazardpass', 'beskyttelsesnet', 'bringerofdarkness', 'soulbend', 'smokebomb', 'hurl'];
			...
			onStart(battle, source, effect) {
				...
				for (const sideCondition of removeAll) {
					if (source.side.foe.removeSideCondition(sideCondition)) {
						if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side.foe, this.dex.conditions.get(sideCondition).name, '[from] move: Prismatic Terrain', '[of] ' + source);
					}
					if (source.side.removeSideCondition(sideCondition)) {
						if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side, this.dex.conditions.get(sideCondition).name, '[from] move: Prismatic Terrain', '[of] ' + source);
					}
				}
			},
			onResidualOrder: 21,
	...
	corrosivetoxic: {
		...
		onTryHit(target, source, move) {
			...
			if (target.hasType('Steel') || target.hasType('Poison')) {
				...
				const status = this.dex.conditions.get(move.status);
				target.status = status.id;
				target.statusState = {id: status.id, target: target, source: source, stage: 0};
	...
	fakeclaim: {
		...
		onModifyMove(move) {
			if (move.baseMove) {
				const baseMove = this.dex.moves.get(move.baseMove);
				move.type = baseMove.type;
				move.basePower = baseMove.basePower;
				move.category = baseMove.category;
			}
		},
		onTryMove() {
			this.attrLastMove('[still]');
		...
		onPrepareHit(target, source, move) {
			const zmove = this.dex.moves.get(this.actions.Z_MOVES[move.type]);
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			...
			const baseMove = move.baseMove || 'bravebird';
			...
			for (const newMove of set.moves) {
				const moveTemplate = this.dex.moves.get(newMove);
		...
		onHit(target, source) {
			...
			for (const i in sideConditions) {
				...
				if (source.side.removeSideCondition(i)) {
					this.add('-sideend', source.side, this.dex.conditions.get(i).name, '[from] move: Smoke Bomb', '[of] ' + source);
	...
	holyorders: {
		...
		onHit(target, source) {
			const orders = ["healorder", "defendorder", "attackorder"];
			this.prng.shuffle(orders);
			this.actions.useMove(orders[0], source);
			this.actions.useMove(orders[1], source);
		},
		secondary: null,
		target: "self",
		type: "Bug",
		...
		condition: {
			...
			onDisableMove(pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					if (this.dex.moves.get(moveSlot.id).category === 'Status' && moveSlot.id !== 'purplepills') {
