	...
	*/
	...
	evoblast: {
		...
		onAfterMoveSecondarySelf(pokemon) {
			const stat = (['atk', 'def', 'spa', 'spd', 'spe', 'accuracy'] as BoostName[])[this.random(6)];
			const boost: SparseBoostsTable = {};
			boost[stat] = 1;
			...
			onHit(pokemon) {
				...
				for (const condition of sideConditions) {
					if (pokemon.hp && pokemon.side.removeSideCondition(condition)) {
						this.add('-sideend', pokemon.side, this.dex.getEffect(condition).name, '[from] move: Cyclone Spin', '[of] ' + pokemon);
					}
				}
			...
			onStart(battle, source, effect) {
				...
				for (const sideCondition of removeAll) {
					if (source.side.foe.removeSideCondition(sideCondition)) {
						if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side.foe, this.dex.getEffect(sideCondition).name, '[from] move: Prismatic Terrain', '[of] ' + source);
					}
					if (source.side.removeSideCondition(sideCondition)) {
						if (!(silentRemove.includes(sideCondition))) this.add('-sideend', source.side, this.dex.getEffect(sideCondition).name, '[from] move: Prismatic Terrain', '[of] ' + source);
					}
				}
			},
			onResidualOrder: 21,
	...
	corrosivetoxic: {
		...
		onTryHit(target, source, move) {
			...
			if (target.hasType('Steel') || target.hasType('Poison')) {
				...
				const status = this.dex.getEffect(move.status);
	...
	fakeclaim: {
		...
		onModifyMove(move) {
			if (move.baseMove) {
				const baseMove = this.dex.getMove(move.baseMove);
		...
		onAfterMoveSecondarySelf(pokemon, target, move) {
			...
			for (const newMove of set.moves) {
				const moveTemplate = this.dex.getMove(newMove);
	...
	armyofmushrooms: {
		...
		beforeTurnCallback(pokemon) {
			...
			this.boost({def: 1, spd: 1}, pokemon, pokemon, this.dex.getEffect('mushroom army'));
		...
		onAfterHit(target, source, move) {
			...
			case 2:
				if (option) {
					...
					const targetAbility = this.dex.getAbility(target.ability);
					const sourceAbility = this.dex.getAbility(source.ability);
	...
	holyorders: {
		...
		onHit(target, source) {
			...
			this.useMove(orders[0], source);
			this.useMove(orders[1], source);
		},
		secondary: null,
		target: "self",
		type: "Bug",
		...
		condition: {
			...
			onDisableMove(pokemon) {
				for (const moveSlot of pokemon.moveSlots) {
					if (this.dex.getMove(moveSlot.id).category === 'Status' && this.dex.getMove(moveSlot.id).id !== 'purplepills') {
