export const Moves: {[k: string]: ModdedMoveData} = {
	...
	conversion2: {
		...
		onHit(target, source) {
			...
			const possibleTypes = [];
			const attackType = this.dex.getMove(target.lastMove).type;
		...
		onHit(target, source) {
			...
			for (const i in sideConditions) {
				if (target.side.removeSideCondition(i)) {
					this.add('-sideend', target.side, this.dex.getEffect(i).name, '[from] move: Defog', '[of] ' + target);
				}
			}
			for (const i in sideConditions) {
				if (i === 'reflect' || i === 'lightscreen') continue;
				if (source.side.removeSideCondition(i)) {
					this.add('-sideend', source.side, this.dex.getEffect(i).name, '[from] move: Defog', '[of] ' + source);
				}
			}
		},
	},
	...
	sleeptalk: {
		...
		onHit(pokemon) {
			...
			for (const move of pokemon.moves) {
				...
				if (move && !(NoSleepTalk[move] || this.dex.getMove(move).flags['charge'])) {
					moves.push(move);
				}
			}
			let move = '';
