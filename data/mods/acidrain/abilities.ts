export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	"flowergift": {
		...
		onAllyModifyAtk(atk) {
			if (this.effectData.target.baseSpecies.speciesid !== 'cherrmin') return;
	...
	"harvest": {
		inherit: true,
		onResidual(pokemon) {
			if (pokemon.hp && !pokemon.item && this.dex.getItem(pokemon.lastItem).isBerry && (!this.field.suppressingWeather() || this.random(2))) {
