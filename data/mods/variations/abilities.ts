export const Abilities: {[k: string]: ModdedAbilityData} = {
	...
	lightningrod: {
		...
		onAnyRedirectTarget(target, source, source2, move) {
			if (move.type !== this.effectState.target.hpType || move.id in {firepledge: 1, grasspledge: 1, waterpledge: 1}) return;
	...
	magnetpull: {
		...
		onFoeTrapPokemon(pokemon) {
			if (pokemon.hasType(this.effectState.target.hpType) && pokemon.isAdjacent(this.effectState.target)) {
				pokemon.tryTrap(true);
			}
		},
		onFoeMaybeTrapPokemon(pokemon, source) {
			if (!source) source = this.effectState.target;
			if (!source?.isAdjacent(pokemon)) return;
	...
	normalize: {
		inherit: true,
		onModifyMove(move, pokemon) {
			if (move.id !== 'struggle' && this.dex.moves.get(move.id).type !== pokemon.hpType) {
				move.type = pokemon.hpType;
			}
		},
	},
