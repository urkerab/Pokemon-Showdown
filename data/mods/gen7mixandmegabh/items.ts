export const Items: {[k: string]: ModdedItemData} = {
	blueorb: {
		...
		onPrimal(pokemon) {
			const template = pokemon.species.baseSpecies === 'Kyogre' ? 'Kyogre-Primal' : pokemon.baseSpecies;
	...
	redorb: {
		...
		onPrimal(pokemon) {
			const template = pokemon.species.baseSpecies === 'Groudon' ? 'Groudon-Primal' : pokemon.baseSpecies;
			pokemon.formeChange(template, this.effect, true);
			if (pokemon.m.originalSpecies !== 'Groudon') {
				pokemon.baseSpecies = pokemon.species;
				pokemon.baseAbility = pokemon.ability;
				this.add('-start', pokemon, 'Red Orb', '[silent]');
				const oTemplate = this.dex.species.get(pokemon.illusion ? pokemon.illusion.m.originalSpecies : pokemon.m.originalSpecies);
				if (pokemon.illusion) {
					pokemon.ability = '';
