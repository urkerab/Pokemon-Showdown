...
export const Formats: (InheritableFormatData | {section: string, column?: number})[] = [
	...
	},
	{
		...
		ruleset: ['[Gen 8] Ubers'],
		banlist: [
			'Pawniard', 'Rufflet', 'Arena Trap', 'Eviolite', 'Light Ball',
		],
	...
	},
	{
		...
		banlist: [
			'Buzzwole', 'Dragapult', 'Gengar', 'Keldeo', 'Magearna', 'Naganadel', 'Noivern', 'Shedinja', 'Urshifu', 'Zeraora', 'Zygarde',
			'Comatose', 'Contrary', 'Fluffy', 'Fur Coat', 'Gorilla Tactics', 'Huge Power', 'Ice Scales', 'Illusion',
			'Imposter', 'Innards Out', 'Intrepid Sword', 'Libero', 'Magnet Pull', 'Neutralizing Gas', 'Parental Bond',
			'Poison Heal', 'Protean', 'Pure Power', 'Simple', 'Speed Boost', 'Stakeout', 'Water Bubble', 'Wonder Guard',
		],
		unbanlist: ['Darmanitan'],
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			if (!this.dex.data.TypeChart[target.set.name]) return;
			return {...template, addedType: target.set.name};
		},
	},
	{
		name: "[Gen 8] Camomons",
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Dracovish', 'Hydreigon', 'Kyurem', 'Shedinja', 'Zeraora'],
		unbanlist: ['Melmetal'],
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const types = [...new Set(target.set.moves.slice(0, 2).map(move => this.dex.getMove(move).type))];
	...
	},
	{
		...
		ruleset: ['[Gen 8] OU'],
		banlist: ['Shedinja', 'Huge Power', 'Neutralizing Gas', 'Focus Sash', 'Bolt Beak', 'Disable', 'Double Iron Bash', 'Fishious Rend', 'Grass Whistle', 'Hypnosis', 'Lovely Kiss', 'Perish Song', 'Sing', 'Sleep Powder', 'Spore', 'Transform'],
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] Ubers', 'Moody Clause'],
		banlist: ['Type: Null', 'Gorilla Tactics', 'Ice Scales'],
		restricted: ['Chansey', 'Shedinja'],
		onValidateTeam(team) {
		...
		validateSet(set, teamHas) {
			const crossTemplate = this.dex.getSpecies(set.name);
			...
			const crossPrevoTemplate = this.dex.getSpecies(crossTemplate.prevo);
		...
		onModifySpecies(template, target, format, effect) {
			...
			const crossTemplate = this.dex.getSpecies(target.set.name);
			...
			mixedTemplate.baseSpecies = mixedTemplate.species = template.name + '-' + crossTemplate.name;
	...
	},
	{
		...
		onModifySpecies(template, target, source, effect) {
			...
			const spe = this.dex.getSpecies(target.set.species).baseStats.spe;
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Huge Power', 'Uber > 1', 'Uber ++ Power Construct'],
		unbanlist: ['Uber'],
		onModifySpecies(template, target, format, effect) {
			if (!target.side) return;
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const uber = target.side.team.find(set => this.dex.getSpecies(set.species).tier === 'Uber') || target.side.team[0];
			const stat = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'][target.side.team.indexOf(target.set)] as StatName;
			const newTemplate = this.dex.deepClone(template);
			newTemplate.baseStats[stat] = this.dex.getSpecies(uber.species).baseStats[stat];
	...
	},
	{
		...
		getEvoFamily(species) {
			let template = Dex.getSpecies(species);
		...
		validateSet(set, teamHas) {
			...
			if (!this.format.abilityMap) {
				for (const speciesid in this.dex.data.Pokedex) {
					...
					if (this.dex.data.Pokedex[speciesid].requiredAbility || this.dex.data.Pokedex[speciesid].requiredItem || this.dex.data.Pokedex[speciesid].requiredMove || this.dex.data.Pokedex[speciesid].battleOnly || ["Gmax", "Antique"].includes(this.dex.data.Pokedex[speciesid].forme)) continue;
			...
			const template = this.dex.getSpecies(species);
			const name = set.name;
			...
			for (const speciesid of pokemonWithAbility) {
				const donorTemplate = this.dex.getSpecies(speciesid);
		...
		onValidateTeam(team, format, teamHas) {
			...
			for (const evoFamily of evoFamilyLists) {
				...
				if (requiredFamilies[evoFamilies[0]]) return ["You are limited to one inheritance from each family by the Donor Clause.", "(You inherit more than once from " + this.dex.getSpecies(evoFamilies[0]).name + "'s.)"];
	...
	},
	{
		...
		banlist: ['Diggersby', 'Dracozolt', 'Kartana', 'Naganadel', 'Pheromosa', 'Porygon-Z', 'Regieleki', 'Rillaboom', 'Spectrier'],
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] Ubers'],
		banlist: ['Eternatus', 'Lunala', 'Zacian-Crowned', 'Moody', 'Electrify'],
		restricted: [
			'Beedrillite', 'Blazikenite', 'Gengarite', 'Kangaskhanite', 'Mawilite', 'Medichamite', 'Pidgeotite',
			'Uber', 'Gengar', 'Zeraora',
		],
		unbanlist: [
			'Darmanitan', 'Dracovish', 'Lunala', 'Zamazenta',
			'Abomasnow-Mega', 'Charizard-Mega-X', 'Charizard-Mega-Y', 'Gardevoir-Mega', 'Gallade-Mega',
			'Glalie-Mega', 'Gyarados-Mega', 'Lucario-Mega', 'Manectric-Mega', 'Steelix-Mega', 'Tyranitar-Mega',
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (this.dex.getItem(item).megaStone) return ["You are limited to one of each Mega Stone.", "(You have more than one " + this.dex.getItem(item).name + ".)"];
			}
		},
		onValidateSet(set) {
			const template = this.dex.getSpecies(set.species || set.name);
			const item = this.dex.getItem(set.item);
			if (!item.megaStone) return;
			if (this.ruleTable.isRestrictedSpecies(template)) {
				return [template.name + " is not allowed to hold a Mega Stone."];
			}
			if (template.baseSpecies === item.megaEvolves) return;
			const ability = this.dex.getSpecies(item.megaStone).abilities[0];
		...
		onModifySpecies(template, target, format, effect) {
			if (!effect || effect.effectType !== 'Item') return;
		...
		onSwitchIn(pokemon) {
			const oMegaTemplate = this.dex.getSpecies(pokemon.species.originalMega);
		...
		battle: {
			...
			runMegaEvo(pokemon) {
				...
				const oMegaTemplate = this.dex.getSpecies(pokemon.canMegaEvo);
				const effect = this.dex.getItem(oMegaTemplate.requiredItem);
			...
			getMegaDeltas(megaTemplate) {
				const baseSpecies = this.dex.getSpecies(megaTemplate.baseSpecies);
			...
			doGetMixedSpecies(template, deltas) {
				...
				if (newTemplate.types[0] === deltas.type) {
		...
		pokemon: {
			getItem() {
				const item = this.battle.dex.getItem(this.item);
				if (item.megaStone) {
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const newTemplate = this.dex.deepClone(template);
			const stats: StatNameExceptHP[] = ['atk', 'def', 'spa', 'spd', 'spe'];
	...
	},
	{
		...
		banlist: [
			...
			'Simple', 'Slush Rush', 'Speed Boost', 'Tinted Lens', 'Trace', 'Unaware', 'Unburden', 'Water Bubble',
	...
	},
	{
		...
		mod: 'gen8',
		ruleset: ['[Gen 8] OU'],
		banlist: ['Cinderace', 'Dragapult', 'Kartana', 'Porygon-Z', 'Rillaboom', 'Shedinja', 'Urshifu-Single-Strike', 'Zeraora', "King's Rock"],
		...
		validateSet(set, teamHas) {
			const problems = this.validateSet(set, teamHas);
			for (set.sketchMove of set.moves) {
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU'],
		banlist: ['Dragapult', 'Landorus-Therian', 'Mamoswine', 'Naganadel', 'Silvally', 'Thundurus-Base', "King's Rock", 'Razor Fang'],
		unbanlist: ['Melmetal'],
		restricted: ['Acupressure', 'Astral Barrage', 'Belly Drum', 'Bolt Beak', 'Double Iron Bash', 'Electrify', 'Extreme Speed', 'Fishious Rend', 'Geomancy', 'Lovely Kiss', 'Shell Smash', 'Shift Gear', 'Spore', 'Thousand Arrows', 'V-Create', 'Wicked Blow'],
		checkCanLearn(move, template, lsetData, set) {
			if (!move.isMax && !this.ruleTable.isRestricted('move:' + move.id)) {
				let types = template.types;
				if (template.prevo) types = types.concat(this.dex.getSpecies(this.dex.getSpecies(template.prevo).prevo || template.prevo).types);
	...
	},
	{
		...
		ruleset: ['[Gen 7] Ubers', '!OHKO Clause'],
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				while (template.prevo) {
					template = this.dex.getSpecies(template.prevo);
				}
	...
	},
	{
		...
		ruleset: ['[Gen 8] OU'],
		banlist: [],
		restricted: ['Baneful Bunker', 'Block', 'Copycat', 'Destiny Bond', 'Detect', 'Fairy Lock', 'Ingrain', 'Instruct', "King's Shield", 'Mean Look', 'move:Metronome', 'Nature Power', 'Obstruct', 'Octolock', 'Parting Shot', 'Protect', 'Roar', 'Skill Swap', 'Spiky Shield', 'Sleep Talk', 'Teleport', 'Whirlwind', 'Wish', 'Yawn'],
		onValidateTeam(team, format, teamHas) {
		...
		validateSet(set, teamHas) {
			const move = this.dex.getMove(set.ability);
		...
		pokemon: {
			getAbility() {
				const move = this.battle.dex.getMove(this.ability);
				if (!move.exists) return this.battle.dex.getAbility(this.ability);
	...
	},
	{
		...
		mod: 'gen7',
		gameType: 'doubles',
		ruleset: ['Obtainable', 'Standard Doubles'],
		banlist: ['Dark Void'],
	},
	{
	...
	},
	{
		...
		onValidateSet(set, format) {
			const template = this.dex.getSpecies(set.species || set.name);
			if (!template.otherFormes || !this.dex.getSpecies(template.otherFormes[0]).isMega) {
	...
	},
	{
		...
		ruleset: ['Obtainable', 'Species Clause', 'Nickname Clause', 'OHKO Clause', 'Evasion Moves Clause', 'Accuracy Moves Clause', 'Swagger Clause', 'HP Percentage Mod', 'Cancel Mod', 'Team Preview'],
	...
	},
	{
		...
		validateTeam(team, removeNicknames) {
			if (team.length !== 1 || !['eevee', 'pikachu'].includes(this.toID(team[0].species))) return ['You may only bring either Pikachu or Eevee.'];
	...
	},
	{
		...
		banlist: [
			...
			'Lopunny-Mega', 'Magearna', 'Magnezone', 'Mawile-Mega', 'Metagross-Mega', 'Mew',
	...
	},
	{
		...
		mod: 'gen7',
		ruleset: ['Obtainable', 'Standard', 'Evasion Abilities Clause', 'Baton Pass Clause'],
	...
	},
	{
		...
		banlist: ['Groudon-Primal', 'Rayquaza-Mega', 'Arena Trap', 'Huge Power', 'Illusion', 'Innards Out', 'Magnet Pull', 'Moody', 'Parental Bond', 'Protean', 'Psychic Surge', 'Pure Power', 'Shadow Tag', 'Stakeout', 'Water Bubble', 'Wonder Guard', 'Chatter', 'Comatose + Sleep Talk'],
	...
	},
	{
		...
		onBeforeFaint(pokemon) {
			const prevo = pokemon.baseSpecies.isMega ? this.dex.getSpecies(pokemon.baseSpecies.baseSpecies).prevo : pokemon.baseSpecies.prevo;
			if (!prevo || pokemon.set.ability === 'Battle Bond') return;
			let template = this.dex.getSpecies(pokemon.set.species);
			let abilitySlot = (Object.keys(template.abilities) as AbilityIndex[]).find(slot => template.abilities[slot] === pokemon.set.ability);
			template = this.dex.getSpecies(prevo);
			...
			const learnset = this.dex.getLearnsetData(template.id).learnset || this.dex.getLearnsetData(this.toID(template.baseSpecies)).learnset || {};
			const prevoset = template.prevo && this.dex.getLearnsetData(this.toID(template.prevo)).learnset || {};
	...
	},
	{
		...
		ruleset: ['Obtainable', 'Moody Clause', 'OHKO Clause', 'Evasion Moves Clause', 'Endless Battle Clause', 'HP Percentage Mod', 'Cancel Mod', 'Team Preview'],
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const dual = this.dex.getItem(set.ability);
			...
			if (this.toID(set.item) === this.toID(set.ability)) problems.push((set.name || this.dex.getSpecies(set.species).name) + " is limited to 1 of " + this.dex.getItem(set.item).name);
			else if (this.toID(set.item).startsWith('choice') && this.toID(set.ability).startsWith('choice')) problems.push((set.name || this.dex.getSpecies(set.species).name) + " has the combination of " + this.dex.getItem(set.item).name + " and " + dual.name + ", which is banned.");
			return problems.length ? problems : null;
		},
	},
	{
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const item = set.item;
			const move = this.dex.getMove(set.item);
		...
		onBegin() {
			for (const pokemon of this.p1.pokemon.concat(this.p2.pokemon)) {
				const move = this.dex.getMove(pokemon.set.item);
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const uber = target.side.team.find(set => {
				const item = this.dex.getItem(set.item);
				return this.toID(set.ability) === 'powerconstruct' || this.toID(set.ability) === 'shadowtag' || this.dex.getSpecies(item.megaEvolves === set.species ? item.megaStone : set.species).tier === 'Uber';
			}) || target.side.team[0];
			const stat = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'][target.side.team.indexOf(target.set)] as StatName;
			const newTemplate = this.dex.deepClone(template);
			newTemplate.baseStats[stat] = this.dex.getSpecies(uber.species).baseStats[stat];
		...
		validateSet(set, teamHas) {
			if (!teamHas.typeTable) {
				...
				if (template.name === item.megaEvolves) {
					template = this.dex.getSpecies(item.megaStone);
			...
			if (this.activeMove?.hotPotato && target.hp) {
				...
				for (const sideCondition of ['stealthrock', 'spikes', 'toxicspikes', 'stickyweb']) {
					...
					if (oldData) {
						...
						this.add('-sideend', source.side, this.dex.getEffect(sideCondition).name);
				...
				if (source.status) {
					const oldData = source.statusData;
					source.cureStatus();
					if (target.trySetStatus(oldData.id, source)) {
						for (const i in oldData) {
							if (!noCopy.includes(i)) target.statusData[i] = oldData[i];
						}
					}
				}
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			const template = this.dex.getSpecies(species);
			const item = this.dex.getItem(set.item);
			const megaTemplate = item.megaEvolves === set.species ? this.dex.getSpecies(item.megaStone) : template;
	...
	},
	{
		...
		onBeforeFaint(target) {
			...
			this.runMove(target.moves[target.moves.length - 1] as string, target, 0, this.format, undefined, true);
		},
	},
	{
		name: "[Gen 7] Linked",
	...
	},
	{
		...
		onValidateSet(set) {
			...
			let template = this.dex.getSpecies(set.species);
			const item = this.dex.getItem(set.item);
			if (item.megaEvolves === template.name) template = this.dex.getSpecies(item.megaStone);
	...
	},
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (++itemTable[item] < 2) continue;
				if (this.dex.getItem(item).megaStone) return ["You are limited to one of each Mega Stone.", "(You have more than one " + this.dex.getItem(item).name + ".)"];
				if (item === 'blueorb' || item === 'redorb') return ["You are limited to one of each Primal Orb.", "(You have more than one " + this.dex.getItem(item).name + ".)"];
			}
		},
		onValidateSet(set) {
			const template = this.dex.getSpecies(set.species || set.name);
	...
	},
	{
		...
		inherit: ["[Gen 7] Mix and Mega"],
		banlist: [
			'Blissey', 'Breloom', 'Buzzwole', 'Charizard-Mega-X', 'Darkrai',
			'Entei', 'Ferrothorn', 'Garchomp', 'Genesect', 'Golisopod', 'Greninja-Ash', 'Groudon-Primal', 'Gyarados', 'Heatran',
			'Kartana', 'Kyogre-Primal', 'Lunala', 'Magearna', 'Mandibuzz', 'Mew',
			'Mimikyu', 'Noivern', 'Raikou', 'Shuckle',
			'Tapu Koko', 'Tapu Lele', 'Terrakion', 'Toxapex', 'Victini', 'Volcarona', 'Weavile', 'Xerneas', 'Zapdos', 'Zeraora', 'Zygarde',
		],
	},
	{
		name: "[Gen 7] Monotype",
	...
	},
	{
		...
		onModifyMove(move, pokemon) {
			...
			const moveCopy = this.dex.getMove(move.id);
	...
	},
	{
		...
		onBegin() {
			const allPokemon = this.p1.pokemon.concat(this.p2.pokemon);
			for (const pokemon of allPokemon) {
				if (pokemon.ability === pokemon.species.abilities['S']) {
					pokemon.m.innates = [];
					continue;
				}
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const newTemplate = this.dex.deepClone(template);
			if (target.set.item) {
				const item = this.dex.getItem(target.set.item);
				if (item.megaEvolves === template.name) newTemplate.tier = this.dex.getSpecies(item.megaStone).tier;
			}
			if (newTemplate.tier[0] === '(') newTemplate.tier = newTemplate.tier.slice(1, -1);
			if (!(newTemplate.tier in boosts)) return;
	...
	},
	{
		...
		ruleset: ['[Gen 7] Doubles OU', 'Accuracy Moves Clause', 'Sleep Clause Mod', 'Z-Crystals Clause'],
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['Obtainable', 'Moody Clause', 'HP Percentage Mod', 'Cancel Mod'],
		banlist: [
		...
		onValidateSet(set) {
			...
			const item = this.dex.getItem(set.item);
			if (item.megaEvolves === template.name) template = this.dex.getSpecies(item.megaStone);
			if (this.toID(set.ability) === 'battlebond' && template.id === 'greninja') template = this.dex.getSpecies('greninjaash');
	...
	},
	{
		...
		searchShow: true,
		ruleset: ['[Gen 8] OU', '2 Ability Clause', '!Obtainable Abilities'],
		banlist: ['Buzzwole', 'Dragapult', 'Gengar', 'Keldeo', 'Magearna', 'Naganadel', 'Noivern', 'Shedinja', 'Urshifu', 'Zeraora', 'Zygarde'],
		unbanlist: ['Darmanitan'],
		restricted: [
			'Comatose', 'Contrary', 'Fluffy', 'Fur Coat', 'Gorilla Tactics', 'Huge Power', 'Ice Scales', 'Illusion',
			'Imposter', 'Innards Out', 'Intrepid Sword', 'Libero', 'Magnet Pull', 'Neutralizing Gas', 'Parental Bond',
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const fuseTemplate = this.dex.getSpecies(target.set.name);
	...
	},
	{
		...
		banlist: ['Bright Powder', 'Focus Band', "King's Rock", 'Lax Incense', 'Quick Claw'],
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			if (template.battleOnly) baseStats.hp = target.baseSpecies.baseStats.hp;
	...
	},
	{
		...
		banlist: ['TR29'],
		onValidateSet(set) {
			const moveid = this.dex.getItem(set.item).moveid;
			if (moveid) {
				let species = this.dex.getSpecies(set.species);
				let lsetData = this.dex.getLearnsetData(species.id);
				if (!lsetData.learnset) {
					species = this.dex.getSpecies(species.changesFrom || species.baseSpecies);
					lsetData = this.dex.getLearnsetData(species.id);
				}
				if (!lsetData.learnset?.[moveid]?.includes("8M")) return [(set.species || set.name) + ' cannot learn ' + this.dex.getMove(moveid).name + '.'];
			}
		},
		onBegin() {
			for (const pokemon of this.p1.pokemon.concat(this.p2.pokemon)) {
		...
		battle: {
			makeRequest(type) {
				...
				case 'teampreview':
					...
					let maxTeamSize = teamLengthData?.battle;
	...
	},
	{
		...
		validateSet(set, teamHas) {
			if (set.moves.length <= 4) {
				...
				set.moves = set.moves.filter(move => {
					const ability = this.dex.getAbility(move);
		...
		onBegin() {
			for (const pokemon of this.p1.pokemon.concat(this.p2.pokemon)) {
				pokemon.baseMoveSlots = pokemon.baseMoveSlots.filter(slot => !this.dex.getAbility(slot.id).exists);
				pokemon.moveSlots = pokemon.baseMoveSlots.slice();
			}
		},
		onSwitchIn(pokemon) {
		...
		checkCanLearn(move, template, lsetData, set) {
			...
			if (!this.ruleTable.isRestricted('ability:' + ability.id)) {
				for (const speciesid in this.dex.data.Learnsets) {
					const pokemon = this.dex.data.Pokedex[speciesid] || this.dex.getSpecies(speciesid);
					if (pokemon.num < 1 || pokemon.num > 807) continue;
					...
					if (this.checkCanLearn(move, this.dex.getSpecies(speciesid))) continue;
					return null;
				}
			}
			return this.checkCanLearn(move, template, lsetData, set);
	...
	},
	{
		...
		onAfterMega(pokemon) {
			if (pokemon.m.innate === this.toID(pokemon.ability)) {
	...
	},
	{
		...
		ruleset: ['[Gen 7] OU'],
		onModifyMove(move, pokemon, target) {
			move.type1 = this.dex.getMove(pokemon.moves[1]).type;
			move.type3 = this.dex.getMove(pokemon.moves[3]).type;
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const baseSpecies = template.baseSpecies ? this.dex.getSpecies(template.baseSpecies) : template;
			if (!baseSpecies.prevo) return;
			let prevoTemplate = this.dex.getSpecies(baseSpecies.prevo);
			if (prevoTemplate.prevo) prevoTemplate = this.dex.getSpecies(prevoTemplate.prevo);
	...
	},
	{
		...
		validateSet(set, teamHas) {
			...
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
			return new TeamValidator(Dex.getFormat(this.format.id + '@@@' + customRules.join(','))).validateSet(set, teamHas);
		},
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
		...
		onFaint(pokemon, source, effect) {
			if (source && effect && effect.effectType === 'Move' && !effect.isFutureMove) {
				...
				if (pokemon.lastMove && pokemon.lastMove.id && !source.baseMoves.includes(pokemon.lastMove.id)) {
					const move = this.dex.getMove(pokemon.lastMove.id);
					const metaMove = {
						move: move.name,
						id: move.id,
						pp: move.pp,
	...
	{
		name: "[Gen 7] Anything Goes Mix and Mega",
	...
	},
	{
		...
		banlist: ['Scyther', 'Arena Trap', 'Huge Power', 'Innards Out', 'Magnet Pull', 'Moody', 'Parental Bond', 'Protean', 'Psychic Surge', 'Pure Power', 'Shadow Tag', 'Stakeout', 'Water Bubble', 'Wonder Guard', 'Chatter', 'Comatose + Sleep Talk'],
		unbanlist: [],
	},
	{
		...
		mod: 'gen7mixandmega',
		maxLevel: 5,
		ruleset: ['Obtainable', 'Standard', 'Swagger Clause', 'Little Cup'],
		banlist: ['Gligar', 'Gothita', 'Misdreavus', 'Murkrow', 'Scyther', 'Sneasel', 'Tangela', 'Vulpix', 'Electrify', 'Gengarite', 'Kangaskhanite', 'Dragon Rage', 'Sonic Boom'],
		...
		onValidateSet(set) {
			...
			case 'beedrillite':
				...
				return ["You are only allowed to hold Beedrillite if your Ability is Adaptability."];
			case 'blazikenite':
				if (set.ability === 'Speed Boost') break;
				return ["You are only allowed to hold Blazikenite if your Ability is Speed Boost."];
			case 'gengarite':
			...
			case 'mawilite': case 'medichamite':
				...
				return ["You are only allowed to hold " + item.name + " if your Ability is Huge Power or Pure Power."];
			case 'pidgeotite':
				if (set.ability === 'No Guard') break;
				return ["You are only allowed to hold Pidgeotite if your Ability is No Guard."];
			case 'ultranecroziumz':
				return ["Ultranecrozium Z is only allowed to be held by Necrozma-Dawn-Wings or Necrozma-Dusk-Mane."];
			}
		},
	...
	},
	{
		...
		onValidateSet(set, format, setHas, teamHas) {
			if (!teamHas.typeMap) {
				...
				for (const type in this.dex.data.TypeChart) teamHas.typeMap.set(type, []);
			}
			let template = this.dex.getSpecies(set.species);
			if (!template.types) return [`Invalid pokemon ${set.name || set.species}`];
			...
			for (const [type, problems] of teamHas.typeMap) {
				for (const ban of format.customBans![type]) {
					if (setHas[this.dex.validateBanRule(ban)]) problems.push(`${ban} is banned on ${type} teams.`);
				}
			}
		},
		onValidateTeam(team, format, teamHas) {
	...
	},
	{
		...
		mod: 'multibility',
		ruleset: ['[Gen 7] Ubers', '2 Ability Clause'],
		banlist: ['Kyurem-Black', 'Porygon-Z', 'Togekiss', 'Shedinja', 'Chatter', 'Arena Trap', 'Comatose', 'Contrary', 'Fluffy', 'Fur Coat', 'Huge Power', 'Imposter', 'Innards Out', 'Parental Bond', 'Pure Power', 'Simple', 'Stakeout', 'Water Bubble', 'Wonder Guard'],
		unbanlist: ['Deoxys-Defense', 'Deoxys-Speed', 'Genesect', 'Greninja', 'Landorus'],
		validateSet(set, teamHas) {
			if (!this.dex.getAbility(set.item).exists) return this.validateSet(set, teamHas);
			const problems = this.validateSet({...set, item: ''}, teamHas);
			if (problems) return problems;
			const customRules = this.format.customRules || [];
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
			return new TeamValidator(Dex.getFormat(this.format.id + '@@@' + customRules.join(','))).validateSet({...set, ability: set.item, item: ''}, teamHas);
		},
		onValidateSet(set) {
			const ability = this.dex.getAbility(set.ability);
			const item = this.dex.getItem(set.item);
		...
		onFaint(pokemon) {
			this.singleEvent('End', this.dex.getItem(pokemon.item), pokemon.itemData, pokemon);
		},
		onSwitchOut(pokemon) {
			this.singleEvent('End', this.dex.getItem(pokemon.item), pokemon.itemData, pokemon);
		},
	...
	},
	{
		...
		onValidateSet(set, format, setHas, teamHas) {
			...
			if (teamHas['move:' + this.toID(set.moves[0])] > 2) return ['Your team has the combination of ' + this.dex.getMove(set.moves[0]).name + ' > 2, which is banned.'];
		},
	},
	{
	...
	},
	{
		...
		ruleset: ['[Gen 7] Ubers'],
		onValidateSet(set) {
			if (this.dex.getSpecies(set.species).tier === 'Uber' && set.level > 90) set.level = 90;
		},
	},
	{
	...
	},
	{
		...
		checkCanLearn(move, template, lsetData, set) {
			if (!move.isZ && !this.ruleTable.isRestricted('move:' + move.id) && move.type === this.dex.getItem(set!.item).onPlate) return null;
			return this.checkCanLearn(move, template, lsetData, set);
		},
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
	...
	},
	{
		...
		ruleset: ['Obtainable', 'Standard', 'Accuracy Moves Clause', 'Swagger Clause', 'Z-Crystals Clause'],
	...
	},
	{
		...
		checkCanLearn(move, template, lsetData, set) {
			...
			if (template.prevo && this.dex.getImmunity(move, this.dex.getSpecies(template.prevo)) && this.dex.getEffectiveness(move, this.dex.getSpecies(template.prevo)) > 0) return null;
			return this.checkCanLearn(move, template, lsetData, set);
		},
	},
	{
	...
	},
	{
		...
		onModifySpecies(template, target, effect) {
			...
			if (target.set.item) {
				const item = this.dex.getItem(target.set.item);
				if (item.megaEvolves === newTemplate.species) newTemplate.tier = this.dex.getSpecies(item.megaStone).tier;
			}
			if (newTemplate.tier[0] === '(') newTemplate.tier = newTemplate.tier.slice(1, -1);
			if (!(newTemplate.tier in boosts)) return;
			if (target.set.moves.includes('chatter')) newTemplate.tier = 'PUBL';
		...
		onBegin() {
			for (const side of this.sides) {
				...
				for (const moveid of moves) {
					const move = this.dex.getMove(this.dex.getMove(moveid).id);
					baseMoveSlots.push({
						move: move.name,
						id: move.id,
						pp: 1,
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			...
			if (pokemon.species.isMega && pokemon.types.join() !== this.dex.getSpecies(pokemon.species.baseSpecies).types.join()) return;
		...
		onAfterMega(pokemon) {
			...
			const type = pokemon.side.reflectedType!;
			if (pokemon.types.indexOf(type) > 0 || pokemon.types.length === 1 && pokemon.types[0] === type) return;
			if (pokemon.types.join() !== this.dex.getSpecies(pokemon.species.baseSpecies).types.join()) return;
			if (pokemon.types.length > 1 && pokemon.types[0] === type) {
				pokemon.setType(type);
			} else {
				pokemon.setType([pokemon.types[0], type]);
	...
	},
	{
		...
		battle: {
			boost(boost, target, source, effect, isSecondary, isSelf) {
				...
				const statList: StatNameExceptHP[] = ['atk', 'def', 'spa', 'spd', 'spe'];
	...
	},
	{
		...
		onBeforeMove(attacker, defender, move) {
			...
			if (target) this.useMove(move, target, attacker === defender ? target : attacker);
	...
	},
	{
		...
		banlist: ['Dracovish', 'Dragonite', 'Melmetal', 'Psychic Surge', 'Triage'],
	...
	},
	{
		...
		ruleset: ['[Gen 8] Ubers'],
		unbanlist: ["King's Rock", 'Razor Fang'],
	...
	},
	{
		...
		banlist: ['Archen'],
		unbanlist: ['Drifloon', 'Gothita', 'Swirlix', 'Trapinch', 'Vulpix-Base', 'Wingull', 'Arena Trap', 'Shadow Tag'], // was [],
		restricted: [
	...
	// Ability Shift
	{
		...
		onValidateTeam(team, format) {
			...
			for (const set of team) {
				...
				const sources = pokedex.filter(pokemon => !usedPokemon.includes(pokemon) && this.dex.data.Pokedex[pokemon].num > 0 && template.types.slice().sort().toString() === this.dex.data.Pokedex[pokemon].types.slice().sort().toString() && Object.values(this.dex.data.Pokedex[pokemon].abilities).includes(ability) && this.dex.getSpecies(pokemon).gen <= this.gen);
				if (!sources.length) {
	...
	// Averagemons - Randomised
	{
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				const item = this.dex.getItem(set.item);
				const template = this.dex.getSpecies(item.megaEvolves === set.species ? item.megaStone : set.species);
				total += template.bst;
			}
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const ability = this.dex.getAbility(set.item);
		...
		onBegin() {
			const allPokemon = this.p1.pokemon.concat(this.p2.pokemon);
			for (const pokemon of allPokemon) {
				const ability = this.dex.getAbility(pokemon.item);
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			if (target.set.item) {
				const item = this.dex.getItem(target.set.item);
				if (item.megaEvolves === template.name) tier = this.dex.getSpecies(item.megaStone).tier;
			}
	...
	// Inheritance - Randomised
	{
		...
		onValidateSet(set) {
			...
			const item = this.dex.getItem(set.item);
			if (item.megaEvolves === template.name) template = this.dex.getSpecies(item.megaStone);
			const statTable: Record<StatName, string> = {hp: 'an HP', atk: 'an Attack', def: 'a Defense', spa: 'a Special Attack', spd: 'a Special Defense', spe: 'a Speed'};
			let stat: StatName;
			for (stat in statTable) {
				if (template.baseStats[stat] >= 100) problems.push(template.name + " has " + statTable[stat] + " of " + template.baseStats[stat] + ", which is banned.");
			}
	...
	// Mix and Mega - Randomised
	{
		...
		onValidateTeam(team) {
			if (!team[0]) return;
			const gen = this.dex.getSpecies(team[0].species).gen;
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const nature = this.dex.getNature(target.set.nature);
	...
	},
	{
		...
		onValidateSet(set) {
			return set.moves.map(move => this.dex.getMove(move)).filter(move => move.category === 'Status').map(move => set.species + "'s move " + move.name + " is banned by No Status.");
		},
	},
	...
	},
	{
		...
		ruleset: ['Obtainable', 'Moody Clause', 'OHKO Clause', 'Evasion Moves Clause', 'Accuracy Moves Clause', 'Swagger Clause', 'Endless Battle Clause', 'HP Percentage Mod', 'Cancel Mod', 'Team Preview'],
		banlist: [
	...
	},
	{
		...
		ruleset: ['[Gen 5] OU', '!Evasion Abilities Clause', 'Accuracy Moves Clause'],
	...
	},
	{
		...
		ruleset: ['[Gen 4] OU', 'Accuracy Moves Clause', 'Swagger Clause', 'Team Preview'],
	...
	},
	{
		...
		ruleset: ['[Gen 3] OU', 'Swagger Clause', 'Team Preview'],
	...
	},
	{
		...
		ruleset: ['[Gen 2] OU', 'Swagger Clause', 'Team Preview'],
		banlist: ['Ho-Oh', 'Lugia', 'Mew', 'Mewtwo', 'Snorlax', 'Explosion', 'Flash', 'Hypnosis', 'Kinesis', 'Lovely Kiss', 'Mud-Slap', 'Octazooka', 'Perish Song', 'Sand Attack', 'Self-Destruct', 'Sing', 'Sleep Powder', 'Smokescreen', 'Spore'],
	},
	{
		name: "[Gen 1] 1v1",
		desc: "Bring three Pok&eacute;mon to Team Preview and choose one to battle.",
	...
	},
	{
		...
		banlist: ['Altaria-Mega', 'Charizard-Mega-X', 'Metagross-Mega', 'Sableye-Mega', 'Slowbro-Mega', 'Talonflame', 'Damp Rock', 'Smooth Rock'],
		unbanlist: ['Deoxys-Defense', 'Deoxys-Speed', 'Hoopa-Unbound', 'Landorus', 'Shadow Tag'],
	},
	{
	...
	},
	{
		...
		checkCanLearn(move, template, lsetData, set) {
			if (!move.isZ && !this.ruleTable.isRestricted('move:' + move.id)) {
				let types = template.types;
				if (template.prevo || template.baseSpecies) types = types.concat(this.dex.getSpecies(this.dex.getSpecies(template.prevo || template.baseSpecies).prevo || template.prevo || template.baseSpecies).types);
				if (template.baseSpecies === 'Shaymin') types = ['Grass', 'Flying'];
				if (template.baseSpecies === 'Hoopa') types = ['Psychic', 'Ghost', 'Dark'];
				if (types.includes(move.type)) return null;
			}
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			...
			if (name === 'eeveegeneral') {
				...
				for (const i of ['atk', 'def', 'spa', 'spd', 'spe'] as StatNameExceptHP[]) {
			...
			if (name === 'hashtag') {
				this.boost({spe: 1}, pokemon, pokemon, this.dex.getEffect('Innate Ability'));
			}
			if (name === 'haund') {
				pokemon.addVolatile('prodigy', pokemon);
			}
			if (name === 'innovamania' && !pokemon.illusion) {
				this.boost({atk: 6, def: 6, spa: 6, spd: 6, spe: 6, accuracy: 6}, pokemon, pokemon, this.dex.getEffect('Divine Grace'));
			}
			if (name === 'jackhiggins') {
			...
			if (name === 'marshmallon') {
				this.boost({def: 1}, pokemon, pokemon, this.dex.getEffect('Fur Coat Innate'));
			}
			...
			if (name === 'pikachuun') {
				this.boost({spe: 1}, pokemon, pokemon, this.dex.getEffect('Reisen Cosplay'));
			}
			if (name === 'qtrx') {
			...
			if (name === 'sonired') {
				this.boost({def: -1, spd: -1, atk: 1, spe: 1}, pokemon, pokemon, this.dex.getEffect('Weak Skin'));
			}
			if (name === 'snobalt') {
	...
	},
	{
		...
		onSwitchIn(pokemon) {
			if (pokemon.species.name === 'Magikarp') {
				this.boost({spe: 4, spd: 2, def: 2}, pokemon, pokemon, this.dex.getEffect('The Power of Dank'));
			}
		},
		onModifyMove(move) {
			if (move.id === 'aquaring') {
		...
		onResidual() {
			for (const pokemon of this.getAllActive()) {
				if (pokemon.hp && pokemon.volatiles['wonderring']) {
					this.heal(pokemon.maxhp / 8, pokemon, pokemon, this.dex.getEffect('Dank Memes'));
				}
			}
		},
	},
		...
		validateSet(set, teamHas) {
			...
			for (const speciesid of pokemonWithAbility) {
				...
				if (donorTemplate.id !== this.toID(set.species) && this.format.customBans!.donor.includes(donorTemplate.id)) {
					...
					problems = ["The ability " + this.dex.getAbility(abilityId).name + " is banned from being passed down."];
					continue;
				}
			...
			if (!validSources.length) {
				problems.unshift("" + (set.name || set.species) + " has an illegal set with an ability from " + this.dex.getSpecies(pokemonWithAbility[0]).name);
				return problems;
			}
			return null;
		},
	...
	},
	{
		...
		onDamage(damage, target, source, effect) {
			...
			case 'psn':
				...
				break;
			case 'tox':
				return this.clampIntRange(target.maxhp >> 4 - this.dex.getEffectiveness('Poison', target), 1) * target.statusData.stage;
	...
	},
	{
		...
		onValidateSet(set) {
			...
			if (this.dex.getSpecies(set.species).isNonstandard) problems.push(set.species + ' does not exist.');
			if (this.dex.getAbility(set.ability).isNonstandard) problems.push(this.dex.getAbility(set.ability).name + ' does not exist.');
			const item = this.dex.getItem(set.item);
	...
	},
	{
		...
		banlist: ['Soul Dew'],
		validateSet(set, teamHas) {
			let format: string | Format = '[Gen 6] OU';
			...
			case 'pawn':
				set.name = 'Pawn';
				format = '[Gen 6] LC';
				format = {...Dex.getFormat(format), maxLevel: 100};
				break;
			case 'knight':
				set.name = 'Knight';
				break;
			...
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
			return new TeamValidator(format).validateSet(set, teamHas);
		},
		onValidateTeam(team) {
			...
			if (pieces.has('Knight')) {
				const knight = this.dex.getSpecies(pieces.get('Knight').species);
				let problem = '';
				...
				else if (knight.prevo !== this.toID(pieces.get('Pawn').species) && this.dex.getSpecies(knight.prevo).prevo !== this.toID(pieces.get('Pawn').species)) problem = '' + knight.name + ' does not evolve from ' + pieces.get('Pawn').species + '.';
		...
		onValidateSet(set) {
			if (set.name === 'King') {
				...
				if (this.dex.getItem(set.item).megaEvolves === set.species) return ['Only your King or Queen are allowed to Mega Evolve.'];
			}
		},
		onFaint(pokemon) {
	...
	},
	{
		...
		mod: 'gen6',
		ruleset: ['[Gen 6] Ubers', '!Mega Rayquaza Clause', 'Baton Pass Clause'],
		banlist: [],
		restricted: ['Shedinja'],
		onValidateTeam(team) {
			const nameTable: {[k: string]: boolean} = {};
		...
		validateSet(set, teamHas) {
			...
			const stats: Record<StatName, string> = {'hp': 'HP', 'atk': 'Attack', 'def': 'Defense', 'spa': 'Special Attack', 'spd': 'Special Defense', 'spe': 'Speed'};
			let statid: StatName;
			for (statid in template.baseStats) {
	...
	},
	{
		...
		onSwitchInPriority: 2,
		onSwitchIn(pokemon) {
			const move = this.dex.getMove(pokemon.set.name);
	...
	},
	{
		...
		checkCanLearn(move, template, lsetData, set) {
			...
			if (template.prevo && !this.format.checkCanLearn!.call(this, move, this.dex.getSpecies(template.prevo), lsetData, set)) return null;
			return this.checkCanLearn(move, template, lsetData, set);
		},
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			const uber = target.side.team.find(set => {
				const item = this.dex.getItem(set.item);
				return this.toID(set.ability) === 'shadowtag' || this.dex.getSpecies(item.megaEvolves === set.species ? item.megaStone : set.species).tier === 'Uber';
			}) || target.side.team[0];
			const stat = ['hp', 'atk', 'def', 'spa', 'spd', 'spe'][target.side.team.indexOf(target.set)] as StatName;
			const newTemplate = this.dex.deepClone(template);
			newTemplate.baseStats[stat] = this.dex.getSpecies(uber.species).baseStats[stat];
	...
	},
	{
		...
		onValidateSet(set) {
			const ability = this.dex.getAbility(set.name);
	...
	},
	{
		...
		banlist: ['Fletchling'],
		checkCanLearn(move, template, lsetData, set) {
			return template.evos.every(evo => this.format.checkCanLearn!.call(this, move, this.dex.getSpecies(evo), lsetData, set)) ? this.checkCanLearn(move, template, lsetData, set) : null;
		},
	},
	{
		...
		ruleset: ['[Gen 6] OU'],
		checkCanLearn(move, template, lsetData, set) {
			if (move.id === 'judgment' && this.dex.getItem(set!.item).onPlate) return null;
			return this.checkCanLearn(move, template, lsetData, set);
		},
		...
		onSwitchInPriority: 2,
		onSwitchIn(pokemon) {
			const type = this.dex.getItem(pokemon.item).onPlate;
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			if (!target.side) return;
			if (effect && ['imposter', 'transform'].includes(effect.id)) return;
			const pal = target.side.team.find(set => this.dex.getSpecies(set.species).color === template.color)!;
			return {...template, baseStats: this.dex.getSpecies(pal.species).baseStats};
		},
	},
	{
	...
	},
	{
		...
		ruleset: ['Obtainable', 'Endless Battle Clause', 'HP Percentage Mod', 'Cancel Mod', 'Team Preview', 'Sleep Clause Mod', 'Species Clause'],
	...
	},
	{
		...
		checkCanLearn(move, template, lsetData, set) {
			if (!set!.hpType) {
				...
				if (set!.ivs) {
					let hpTypeX = 0;
					(['hp', 'atk', 'def', 'spe', 'spa', 'spd'] as StatName[]).forEach((stat, i) => { hpTypeX += (set!.ivs[stat] || set!.ivs[stat] === 0 ? set!.ivs[stat] & 1 : 1) << i; });
	...
	},
	{
		...
		validateSet(set, teamHas) {
			const move = this.dex.getMove(set.ability);
			if (move.category !== 'Status' || this.ruleTable.isRestricted('move:' + move.id) || set.moves.map(this.toID).includes(move.id) || this.checkCanLearn(move, this.dex.getSpecies(set.species), undefined, set)) return this.validateSet(set, teamHas);
			set.ability = '';
			const customRules = this.format.customRules || [];
			if (!customRules.includes('!obtainableabilities')) customRules.push('!obtainableabilities');
			const TeamValidator = this.constructor as new(format: string | Format) => TeamValidator;
	...
	},
	{
		...
		onModifySpecies(template, target, format, effect) {
			...
			if (!types.every(type => !!this.dex.data.TypeChart[type])) return;
			return {...template, types};
		},
	},
	{
	...
	},
	{
		...
		mod: 'enchanteditems',
		ruleset: ['Obtainable', 'Standard', 'Swagger Clause', '2 Ability Clause', '!Obtainable Abilities'],
		...
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (item.megaEvolves === template.name) {
					...
					tier = this.dex.getSpecies(species).tier;
				}
				const ability = this.dex.getAbility(set.ability);
